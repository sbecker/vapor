...
describe "/accounts/edit.html.erb" do
  ...
  it "should render edit form" do
    ...
    response.should have_tag("form[action=#{account_path}][method=post]") do
      ...
      with_tag('input#account_aws_secret_access_key[name=?][value=?]', "account[aws_secret_access_key]", template.truncate(@account.aws_secret_access_key, 20))
      with_tag('textarea#account_aws_x_509_key[name=?]', "account[aws_x_509_key]", template.truncate(@account.aws_x_509_key, 60))
      with_tag('textarea#account_aws_x_509_certificate[name=?]', "account[aws_x_509_certificate]", template.truncate(@account.aws_x_509_certificate, 60))
    end
  end
end
